...
@available(iOS 10.0, *)
public class AKNotificationScheduler {
    ...
    public func notificationsQueue() -> [AKNotification] {
        return AKQueue.queue.notificationsQueue()
    }
    

    
    // Cancel the notification if scheduled or queued
    public func cancelNotification (notification: AKNotification) {
        ...
        let queue = AKQueue.queue.notificationsQueue()
        var i = 0
        for noti in queue {
            if notification.identifier == noti.identifier {
                AKQueue.queue.removeAtIndex(i)
                break
            }
            i += 1
        }
    ...
    private func convertToNotificationDateComponent (notification: AKNotification, repeatInterval: RepeatingInterval   ) -> DateComponents {
    ...
    fileprivate func queueNotification (notification: AKNotification) -> String? {
        
        if notification.scheduled {
            return nil
        } else {
            AKQueue.queue.push(notification)
        }
        
        return notification.identifier
    }
    
    public func scheduleNotification ( notification: AKNotification) {
    ...
    public func scheduleAllNotifications () {
        ...
        for _ in queue {
            ...
            if count < min(AKNotificationScheduler.maximumScheduledNotifications, MAX_ALLOWED_NOTIFICATIONS) {
                let popped = AKQueue.queue.pop()
    ...
    fileprivate func scheduleNotificationInternal ( notification: AKNotification) -> String? {
    ...
    public func repeatsFromToDate (identifier: String, alertTitle: String, alertBody: String, fromDate: Date, toDate: Date, interval: Double, repeats: RepeatingInterval, category: String = " ", sound: String = " ") {
        ...
        let notification = AKNotification(identifier: identifier, alertTitle: alertTitle, alertBody: alertBody, date: fromDate, repeats: repeats)
        ...
        for i in 0..<intervalDifference {
            ...
            let notification = AKNotification(identifier: identifier, alertTitle: alertTitle, alertBody: alertBody, date: nextDate, repeats: repeats)
            notification.category = category
            notification.soundName = sound
            self.queueNotification(notification: notification)
        }
    ...
    public func scheduleCategories(categories: [AKCategory]) {
