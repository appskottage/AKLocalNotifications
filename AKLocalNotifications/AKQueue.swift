...
@available(iOS 10.0, *)
public
class AKQueue: NSObject {
    
    internal var notifQueue = [AKNotification]()
    static let queue = AKQueue()
    ...
    internal func push(_ notification: AKNotification) {
    ...
    internal func findInsertionPoint(_ notification: AKNotification) -> Int {
    ...
    internal func pop() -> AKNotification {
        return notifQueue.removeFirst()
    }
    
    //Returns the head of the queue.
    
    internal func peek() -> AKNotification? {
    ...
    internal func notificationsQueue() -> [AKNotification] {
        let queue = notifQueue
        return queue
    }
    
    // Returns DLLocalnotifcation if found, nil otherwise.
    internal func notificationWithIdentifier(_ identifier: String) -> AKNotification? {
    ...
    internal func load() -> [AKNotification]? {
        return NSKeyedUnarchiver.unarchiveObject(withFile: ArchiveURL.path) as? [AKNotification]
    }
...
@available(iOS 10.0, *)
public func <(lhs: AKNotification, rhs: AKNotification) -> Bool {
